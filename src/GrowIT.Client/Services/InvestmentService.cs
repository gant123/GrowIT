...
{
    Task<PaginatedResponse<InvestmentListDto>> GetInvestmentsAsync(InvestmentQueryParams query);
...
public class InvestmentService : BaseApiService, IInvestmentService
{
    ...
    public InvestmentService(HttpClient httpClient) : base(httpClient) { }

    public async Task<PaginatedResponse<InvestmentListDto>> GetInvestmentsAsync(InvestmentQueryParams query)
    {
        var queryString = BuildQueryString(query);
        return await GetAsync<PaginatedResponse<InvestmentListDto>>($"{BaseEndpoint}?{queryString}") 
            ?? new PaginatedResponse<InvestmentListDto>();
    }
